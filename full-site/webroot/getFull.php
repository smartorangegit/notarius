...
notary.sName as notarsName , notary.mName as notarmName, notary.userID as notarID, deal.userID as userID
FROM deal 
INNER JOIN se_services ON se_services.id=deal.serviceID
INNER JOIN common_user ON deal.userID=common_user.userID
LEFT JOIN notary ON (deal.notaryID=notary.id) ORDER BY deal.id DESC') ;
...
$data = array(
    ...
    echo "<h3 class='section-heading'>Таблица всех заявок</h3>";
  ...
  foreach ($msg_info as $item){
      ...
      if($item['transactionStatus']==1){
          // echo    "<a href='/admin/dashboard/new/".$item['deID']."' class='btn btn-primary'>Go</a>";
          $href = "'/admin/dashboard/new/".$item['deID']."'";
      }
      else{
          // echo    "<a href='/admin/dashboard/all/".$item['deID']."' class='btn btn-primary'>Go</a>";
          $href = "'/admin/dashboard/all/".$item['deID']."'";

      }

      echo "<tr class='row100'>";
      echo "<td class='column100 column1' data-column='column1'>";
      echo "<a class='cell__link' href=".$href.">";
      ...
      if(!empty($item['notarfName'])) {
        ...
        echo "<a class='cell__link' href=".$href."><div style='width: 1px; height:20px'></div></a>";
        echo "</td>";
      }
      ...
      echo "<a class='cell__link' href=".$href.">";
      echo  "<p class='option cell_font_family'>";
