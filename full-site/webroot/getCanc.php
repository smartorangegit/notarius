...
echo "<h3 class='section-heading'>Таблица отмененных сделок</h3>";
...
foreach ($msg_info as $item){
    ...
    echo "<a class='cell__link' href='/admin/dashboard/canceled/".$item['deID']."'>";
    ...
    if(!empty($item['notarfName'])) {
        ...
        echo "<a class='cell__link' href='/admin/dashboard/canceled/".$item['deID']."'><div style='width: 1px; height:20px'></div></a>";
        echo "</td>";
    }
    ...
    echo  "<p class='option cell_font_family' id='transactionStatus'>";
        ...
        elseif($item['transactionStatus']==4){echo 'Отмена';}
    echo  "</p>";
    echo "</a>";
